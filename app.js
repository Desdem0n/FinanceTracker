...
const createNewTransaction = () => {
    ...
    <p class="transaction-name">${categoryIcon} ${nameInput.value}</p>
                            ...
                            <button class="delete" onclick="deleteTransaction(${ID})"><i class="fas fa-times"></i>
                                        </button>
                                        </p>
                                        `;
	amountInput.value > 0
...
const checkCategory = transaction => {
	switch (transaction) {
		case "[ + ] Income":
			...
			break;
		case "[ - ] Shopping":
			categoryIcon = '<i class="fas fa-arrow-down"></i>';
			break;
		case "[ - ] Food":
...
const countMoney = money => {
	const newMoney = money.reduce((acc, curr) => acc + curr);
...
const deleteAllTransactions = () => {
	...
	expensesSection.innerHTML = "<h3>Outcome:</h3>";
